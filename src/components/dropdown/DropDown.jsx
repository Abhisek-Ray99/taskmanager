...
const DropDown = ({
  ...
  if(!allProjectNames.some(item => item.name === 'choose project')) {
    allProjectNames = [{name: 'choose project'}, ...allProjectNames ]
  }
  const [selected, setSelected] = useState(allProjectNames[0].name);
  const handleSelect = (newValue) => {
    setSelected(newValue);
    if (newValue === "choose project") {
      onSelect(undefined);
    } else {
      onSelect(newValue);
    }
        ...
        <div className="relative mt-1">
          ...
          >
            <Listbox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm ">
              {allProjectNames.map((project, personIdx) => (
                <Listbox.Option
                  key={personIdx}
                  className={({ active }) =>
